...
Native macOS menu bar app (SwiftUI `MenuBarExtra`) showing live Codex + Codex
...
- `~/.Codex/usage-dashboard/Codex-rate-limits.json`
- `~/.Codex/usage-dashboard/codex-rate-limits.json`

Both files are written by the companion dashboard project at
`~/.Codex/usage-dashboard/` ([theglove44/usage-dashboard](https://github.com/theglove44/usage-dashboard),
own git repo, separate from this one) — statusline hook writes the Codex one on every
render; `codex-live-limits.mjs` / `parse-codex-logs.mjs` write the Codex one).
This app is a pure reader — it never talks to Codex, Codex, or any network
...
- No dependencies — stdlib + SwiftUI + AppKit only. Keep it that way; this
  ...
  trust."
- If the JSON snapshot shape in `~/.Codex/usage-dashboard/*.mjs` changes,
